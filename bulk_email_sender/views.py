...
def index(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            if request.user.email == 'radhikagupta3231@gmail.com' and request.user.username == 'radhikagupta3231':
                ...
                if check_detail.is_valid():
                    ...
                    try:
                        ...
                        server.starttls()
                        server.login('radhikagupta3231@gmail.com','Matrixcoad@80044')
                        ready_msg = MIMEText(request.POST['message'])
                        ready_msg['subject']=request.POST['subject']
                        server.sendmail('radhikagupta3231@gmail.com',request.POST['reciver'],ready_msg.as_string())
                        server.quit()
                ...
                if check_detail.is_valid():
                    ...
                    try:
                        ...
                        for ready_to_send in temp:
                            server.sendmail('radhikagupta3231@gmail.com',str(ready_to_send),ready_msg.as_string())
                            count=count+1
                            print(count)
                        server.quit()
